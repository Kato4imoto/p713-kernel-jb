...
#define AGGRESSIVE_ADJ_SCORE_L1 	(HIDDEN_APP_MIN_ADJ * OOM_SCORE_CAL)
#define AGGRESSIVE_ADJ_SCORE_L2 	(PREVIOUS_APP_ADJ * OOM_SCORE_CAL)
...
#if defined (CONFIG_LGE_AGGRESSIVE_LMK)	
		// set minimum score here (adj score for hidden min(9)). on for loop, biggest oom_socre_adj is selected.
		if (s_miss_cnt > 16  && min_score_adj > AGGRESSIVE_ADJ_SCORE_L1)
		{
			min_score_adj = AGGRESSIVE_ADJ_SCORE_L1;
		}
		// set minimum score here (adj score for previous(7)). on for loop, biggest oom_socre_adj is selected.
		else if (s_miss_cnt > 32  && min_score_adj > AGGRESSIVE_ADJ_SCORE_L2)
		{
			min_score_adj = AGGRESSIVE_ADJ_SCORE_L2;
		}
		s_miss_cnt++;			
#endif
	}
